...
####################  Read in Astrangia Files #################### 
library(tidyverse)

astrangia_results_files <- list.files("Astrangia/", pattern = ".csv") 

list.data<-list()
for (i in 1:length(astrangia_results_files))
{
  list.data[[i]]<-read.delim(paste0("Astrangia/", astrangia_results_files[i]), sep = ",")
}

# add the names of your data to the list
names(list.data)<-paste0("Astrangia_", astrangia_results_files)


for(i in 1:length(results_files)) {                              # Head of for-loop
  assign(paste0("GO_input", i),                                   # Read and store data frames
         read.delim(results_files[i], sep = ","))
}


heat_BvW = read.csv("Astrangia/Sym_state/Heat/heat_sym_results.csv") %>%
  mutate(mutated_p = -log(pvalue)) %>%
  mutate(mutated_p_updown = ifelse(log2FoldChange < 0, mutated_p*-1, mutated_p*1)) %>%
  rename(Protein_ID = X) %>%
  arrange(Protein_ID)

control_BvW = read.csv("Astrangia/Sym_state/Control/sym_control_results.csv") %>%
  mutate(mutated_p = -log(pvalue)) %>%
  mutate(mutated_p_updown = ifelse(log2FoldChange < 0, mutated_p*-1, mutated_p*1)) %>%
  rename(Protein_ID = X) %>%
  arrange(Protein_ID)

cold_BvW = read.csv("Astrangia/") %>%
  mutate(mutated_p = -log(pvalue)) %>%
  mutate(mutated_p_updown = ifelse(log2FoldChange < 0, mutated_p*-1, mutated_p*1)) %>%
  rename(Protein_ID = X) %>%
  arrange(Protein_ID)
...
inputFiles = c(heat_BvW,
               control_BvW,
               cold_BvW);ALTERNATIVE='g'
...
# mucky muck yeeeahhhha
input = "cold_results_modified_pvalues.csv"
goDivision = "CC"
goAnnotations = "astrangia_iso2go.tab"

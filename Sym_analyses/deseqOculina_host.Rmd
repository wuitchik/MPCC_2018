...
expDesign_Host_clones_removed = expDesign_Host %>%
  ...
  filter(genotype!="A") %>%
  filter(genotype!="L") %>%
  filter(genotype!="O") # because we dont have genets represented across treatment

countDataHost_clones_removed = countDataHost %>%
  select(-c(OH15_H_W, OH1_C_W, OH11_F_W, ON4_C_W, ON5_F_W, ON6_H_W, OA4_C_W, OA5_F_W, OA6_H_W, OL6_C_B, OL7_F_B, OL8_H_B, OO1_C_W, OO2_F_W))
...
dds<-DESeqDataSetFromMatrix(countData=countDataHost_clones_removed, colData=expDesign_Host_clones_removed, design=~genotype + treat_type) #can only test for the main effects of treatment
dds = DESeq(dds)
results = results(dds)
summary(results)
```

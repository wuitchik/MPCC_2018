...
But kept the genotype with the highest total read count for all cases.
...
# remove clones from white hosts experimental design and counts
expDesign_Host_white_clones_removed = expDesign_Host_white %>%
  filter(genotype!="H") %>%
  filter(genotype!="N")

countDataHost_white_clones_removed = countDataHost_white %>%
  select(-c(OH15_H_W, OH1_C_W, OH11_F_W, ON4_C_W, ON5_F_W, ON6_H_W))
...
#how many FDR < 10%?
table(res_hot_brown_host$padj<0.05) # we actually get more DEGs when genotype is included in model.
# 0.1=544
# 0.05=338
# 0.01=176
...
host_degs = data.frame(
  temperature = factor(c("hot", "hot", "hot", "hot", "cold", "cold", "cold", "cold")),
  type = factor(c("brown_host", "brown_host", "white_host", "white_host", "brown_host", "brown_host", "white_host", "white_host")),
  direction = factor(c("up","down","up","down","up","down","up","down")),
  temperature_direction = factor(c("hot_up","hot_down", "hot_up","hot_down", "cold_up","cold_down","cold_up","cold_down")),
  number = c(225, -319, 17, -16, 1964, -2375, 1143, -1011)
)

annotate_text = data.frame(
  label = c("1964", "2375", "1143", "1011","225", "319","17", "16"),
  temperature = factor(c("hot", "hot", "hot", "hot", "cold", "cold", "cold", "cold")),
  type = factor(c("brown_host", "brown_host", "white_host", "white_host", "brown_host", "brown_host", "white_host", "white_host")),
  direction = factor(c("up","down","up","down","up","down","up","down")),
  temperature_direction = factor(c("hot_up","hot_down", "hot_up","hot_down", "cold_up","cold_down","cold_up","cold_down")),
  x = c(1,1,1,1,2,2,2,2),
  y = c(1800,-2100,1300,-1200,500,-500,200,-200)
)

cols = c("cold_up" = "#3182bd", "cold_down" =  "#9ecae1","hot_up" = "#de2d26", "hot_down" ="#fc9272")
...
```{r cache=TRUE, fig.align="center"}
cols = c("control" = "grey", "freezing" = "#3182bd", "hot" = "#de2d26")
...
```{r message=FALSE, warning=FALSE, eval = FALSE}
oc_hot_res = read.table("/Users/hannahaichelman/Documents/BU/Host_Buffering/MPCC_2018/Sym_analyses/tables/oculina_hot_brown_host_results.txt")
head(oc_hot_res)
...
head(go_input_hot_oculina)
write.csv(go_input_hot_oculina, file="/Users/hannahaichelman/Documents/BU/Host_Buffering/MPCC_2018/Sym_analyses/tables/oculina_hot_brown_host_GO.csv", quote=F, row.names=FALSE)

```

Make Gene Ontology input table for MWU for Oculina cold treatment
```{r message=FALSE, warning=FALSE, eval = FALSE}
oc_cold_res = read.table("/Users/hannahaichelman/Documents/BU/Host_Buffering/MPCC_2018/Sym_analyses/tables/oculina_cold_brown_host_results.txt")
head(oc_cold_res)
...
head(go_input_cold_oculina)
write.csv(go_input_cold_oculina, file="/Users/hannahaichelman/Documents/BU/Host_Buffering/MPCC_2018/Sym_analyses/tables/oculina_cold_brown_host_GO.csv", quote=F, row.names=FALSE)
```

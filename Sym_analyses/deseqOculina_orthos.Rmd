...
countData <- read.table("/Users/hannahaichelman/Documents/BU/Host_Buffering/MPCC_2018/Sym_analyses/CountsFiles/OrthoCounts_deseq.txt") # clones are already removed from this dataset
```
...
norm.counts.sum = colSums(norm.counts.ortho)
barplot(norm.counts.sum, col=expDesign$temp_type, ylab="counts", main = "Oculina Normalized Counts - Batch Corrected")
...
resBrownHostF.BrownHostC=results(dds, contrast=c("temp_type","cold_brown_host","control_brown_host"), independentFiltering = F)
...
#compare host freezing to host control
resWhiteHostF.WhiteHostC=results(dds, contrast=c("temp_type","cold_white_host","control_white_host"), independentFiltering = F)
...
##second term is the "control"
resSymInHostF.SymInHostC=results(dds, contrast=c("temp_type","cold_sym_inhost","control_sym_inhost"), independentFiltering = F)
